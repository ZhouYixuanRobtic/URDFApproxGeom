...
char qh_version[] = "user_eg 97/3/31 ???"; /* GRB */
...
void qh_qhull(void) {
  ...
  vertices =
      qh_initialvertices(qh hull_dim, maxpoints, qh first_point, qh num_points);
  ...
  if (qh ONLYgood) {
    if (qh GOODvertex > 0 && qh MERGING) {
      fprintf(qh ferr, "qhull input error: 'Qg QVn' (only good vertex) does "
                       "not work with merging.  Use 'Q0'\n");
...
good point (QGn or QG-n), or a good vertex with 'Q0' (QVn).\n");
    ...
    if (qh GOODvertex > 0 && !qh MERGING /* matches qh_partitionall */
      ...
      facet = qh_findbestnew(qh GOODvertexp, qh facet_list, &dist, &isoutside,
                             &numpart);
      zadd_(Zdistgood, numpart);
      if (!isoutside) {
        fprintf(qh ferr,
                "qhull input error: point for QV%d is inside initial simplex\n",
  ...
  qh_settempfree(&vertices);
  qh_settempfree(&maxpoints);
  if (qh PRINToptions1st || qh OUTPUTINFOlevel || qh IStracing) {
    if (qh OUTPUTINFOlevel || qh IStracing)
      fprintf(qh ferr, "\nTrace level %d for %s | %s\n",
              qh IStracing ? qh IStracing : qh OUTPUTINFOlevel, qh rbox_command,
              qh qhull_command);
  ...
  if (!qh STOPpoint && !qh STOPcone) {
    ...
    if (qh ZEROall_ok && !qh TESTvneighbors && !qh WAScoplanar) {
      trace2(
          (qh ferr, "qh_qhull: all facets are clearly convex and no coplanar "
                    "points.  Post-merging and check of maxout not needed.\n"));
    } else {
      ...
      if (qh POSTmerge)
        qh_postmerge("For post-merging", qh postmerge_centrum, qh postmerge_cos,
                     qh TESTvneighbors);
      if (qh visible_list == qh facet_list) { /* i.e., merging done */
        qh findbestnew = True;
        qh_partitionvisible(/*visible_list, newfacet_list*/ !qh_ALL,
                            &numoutside);
...
boolT qh_addpoint(pointT *furthest, facetT *facet, boolT checkdist) {
  ...
  if (checkdist) {
    facet = qh_findbest(furthest, facet, !qh_ALL, False, !qh_NOupper, &dist,
                        &isoutside, &numpart);
  ...
  newbalance = numnew - (realT)(qh num_facets - qh num_visible) * qh hull_dim /
                            qh num_vertices;
  wadd_(Wnewbalance, newbalance);
  wadd_(Wnewbalance2, newbalance * newbalance);
  if (qh ONLYgood && !qh_findgood(qh newfacet_list, goodhorizon) &&
      !qh GOODclosest) {
    FORALLnew_facets qh_delfacet(newfacet);
    qh_delvertex(vertex);
  ...
  qh_resetlists(True /*qh visible_list newvertex_list newfacet_list */);
  trace2((qh ferr,
          "qh_addpoint: added p%d new facets %d new balance %2.2g point "
          "balance %2.2g\n",
...
void qh_buildhull(void) {
  ...
  FORALLfacets {
    if (facet->visible || facet->newfacet) {
      fprintf(qh ferr,
              "qhull internal error (qh_buildhull): visible or new facet f%d "
              "in facet list\n",
              facet->id);
  ...
  FORALLvertices {
    if (vertex->newlist) {
      fprintf(qh ferr,
              "qhull internal error (qh_buildhull): new vertex f%d in vertex "
              "list\n",
              vertex->id);
  ...
  if (qh num_outside && !furthest) {
    fprintf(qh ferr,
            "qhull internal error (qh_buildhull): %d outside points were never "
            "processed.\n",
            qh num_outside);
    qh_errexit(qh_ERRqhull, NULL, NULL);
  }
...
At %02d:%02d:%02d & %2.5g CPU secs, qhull has created %d facets and merged %d.\n\
 The current hull contains %d facets and %d vertices.  Last point was p%d\n",
            tp->tm_hour, tp->tm_min, tp->tm_sec, cpu, qh facet_id - 1, total,
            qh num_facets, qh num_vertices, qh furthest_id);
    return;
  }
  furthestid = qh_pointid(furthest);
  if (qh OUTPUTINFOpoint == furthestid) {
    qh IStracing = qh OUTPUTINFOlevel;
    qhmem.IStracing = qh OUTPUTINFOlevel;
  }
...
At %02d:%02d:%02d & %2.5g CPU secs, qhull has created %d facets and merged %d.\n\
 ...
 outside points.  Next is point p%d (v%d), %2.2g above f%d.\n",
            tp->tm_hour, tp->tm_min, tp->tm_sec, cpu, qh facet_id - 1, total,
            qh num_facets, qh num_vertices, qh num_outside + 1, furthestid,
            qh vertex_id, dist, getid_(facet));
  } else if (qh IStracing >= 1) {
    cpu = qh_CPUclock - qh hulltime;
    cpu /= qh_SECticks;
    qh_distplane(furthest, facet, &dist);
    fprintf(qh ferr,
            "qh_addpoint: add p%d (v%d) to hull of %d facets (%2.2g above f%d) "
            "and %d outside at %4.4g CPU secs.  Previous was p%d.\n",
            furthestid, qh vertex_id, qh num_facets, dist, getid_(facet),
            qh num_outside + 1, cpu, qh furthest_id);
  }
  if (qh visit_id > (unsigned)INT_MAX) {
    qh visit_id = 0;
    FORALLfacets facet->visitid = qh visit_id;
  }
  if (qh vertex_visit > (unsigned)INT_MAX) {
    qh vertex_visit = 0;
    FORALLvertices vertex->visitid = qh vertex_visit;
  }
...
notes:
  similar to delpoint()
*/
void qh_findhorizon(pointT *point, facetT *facet, int *goodvisible,
                    int *goodhorizon) {
    ...
    FOREACHneighbor_(visible) {
      ...
      if (dist > qh MINvisible) {
        ...
        if (dist > -qh MAXcoplanar) {
          ...
          trace2((qh ferr,
                  "qh_findhorizon: point p%d is coplanar to horizon f%d, "
                  "dist=%2.7g < qh MINvisible (%2.7g)\n",
...
Point p%d was above all facets.\n",
  ...
  trace1((qh ferr,
          "qh_findhorizon: %d horizon facets (good %d), %d visible (good %d), "
          "%d coplanar\n",
...
pointT *qh_nextfurthest(facetT **visible) {
  ...
  while ((facet = qh facet_next) != qh facet_tail) {
    ...
    if (qh RANDOMoutside) {
      ...
      fprintf(qh ferr,
              "qhull internal error (qh_nextfurthest): num_outside %d "
              "incorrect or random %2.2g >= 1.0\n",
...
void qh_partitionall(setT *vertices, pointT *points, int numpoints) {
  ...
  trace1(
      (qh ferr, "qh_partitionall: partition all points into outside sets\n"));
...
void qh_partitioncoplanar(pointT *point, facetT *facet, realT *dist) {
  ...
  if (!dist) {
    if (qh findbestnew)
      bestfacet = qh_findbestnew(point, facet, &bestdist, NULL, &numpart);
    else
  ...
  if (istrace) {
    fprintf(qh ferr,
            "qh_partitioncoplanar: ====== p%d increases max_outside to %2.2g "
            "of f%d last p%d\n",
            qh_pointid(point), bestdist, bestfacet->id, qh furthest_id);
    qh_errprint("DISTANT", bestfacet, NULL, NULL, NULL);
  }
  trace4((qh ferr,
          "qh_partitioncoplanar: point p%d is coplanar with facet f%d (or "
          "inside) dist %2.2g\n",
...
void qh_partitionpoint(pointT *point, facetT *facet) {
  ...
  if (qh findbestnew)
    bestfacet = qh_findbestnew(point, facet, &bestdist, &isoutside, &numpart);
  else
  ...
  if (isoutside) {
    if (!bestfacet->outsideset || !qh_setlast(bestfacet->outsideset)) {
    ...
    zinc_(Zpartinside);
    trace4((qh ferr,
            "qh_partitionpoint: point p%d is inside all facets, closest to f%d "
            "dist %2.2g\n",
            qh_pointid(point), bestfacet->id, bestdist));
    if (qh KEEPinside)
      qh_partitioncoplanar(point, bestfacet, &bestdist);
  ...
  *numoutside = 0;
  FORALLvisible_facets {
    ...
    if (visible->outsideset) {
      ...
      *numoutside += size;
      qh num_outside -= size;
      FOREACHpoint_(visible->outsideset) qh_partitionpoint(point, newfacet);
    }
    if (visible->coplanarset &&
        (qh KEEPcoplanar + qh KEEPinside + qh KEEPnearinside)) {
  ...
  trace1((qh ferr,
          "qh_partitionvisible: partitioned %d points from outsidesets and %d "
          "points from coplanarsets\n",
          *numoutside, coplanar));
} /* partitionvisible */
...
Convex hull of %d points in %d-d:\n\n",
  ...
  fprintf(fp, "\nStatistics for: %s | %s", qh rbox_command, qh qhull_command);
  ...
  fprintf(fp, "  Number of distance tests for qhull: %d\n",
          zzval_(Zpartition) + zzval_(Zpartitionall) + zzval_(Znumvisibility) +
              zzval_(Zpartcoplanar));
...
#endif
  nummerged =
      zzval_(Ztotmerge) - zzval_(Zcyclehorizon) + zzval_(Zcyclefacettot);
  if (nummerged) {
    fprintf(fp, "  Number of merged facets: %d\n", nummerged);
    fprintf(fp, "  Number of distance tests for merging: %d\n",
            zzval_(Zbestdist) + zzval_(Zcentrumtests) + zzval_(Zdistconvex) +
                zzval_(Zdistcheck) + zzval_(Zdistzero));
  }
