...
void qh_errexit(int exitcode, facetT *facet, ridgeT *ridge) {
  /*
    if (qh ERREXITcalled) {
      fprintf (qh ferr, "\nqhull error while processing previous error.  Exit
    program\n"); exit(1);
    }
    ...
    fprintf (qh ferr, "\nWhile executing: %s | %s\n", qh rbox_command, qh
    qhull_command); fprintf(qh ferr, "Options selected for %s:\n%s\n",
    qh_version, qh qhull_options); if (qh furthest_id >= 0) { fprintf(qh ferr,
    "Last point added to hull was p%d.", qh furthest_id); if (zzval_(Ztotmerge))
...
*/
void qh_errprint(char *string, facetT *atfacet, facetT *otherfacet,
