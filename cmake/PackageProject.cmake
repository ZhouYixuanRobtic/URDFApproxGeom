# Uses ycm (permissive BSD-3-Clause license) and ForwardArguments (permissive
# MIT license)
...
function(myproject_package_project)
  ...
  set(_oneValueArgs
      ...
      # default to ${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_DATADIR}/${NAME}
      # suitable for vcpkg, etc.
  ...
  set(_multiValueArgs
      ...
      PUBLIC_INCLUDES
      # the names of the INTERFACE/PUBLIC dependencies that are found using
      # `CONFIG`
      PUBLIC_DEPENDENCIES_CONFIGURED
      # the INTERFACE/PUBLIC dependencies that are found by any means using
      # `find_dependency`. the arguments must be specified within double quotes
      # (e.g. "<dependency> 1.0.0 EXACT" or "<dependency> CONFIG").
      PUBLIC_DEPENDENCIES
      # the names of the PRIVATE dependencies that are found using `CONFIG`.
      # Only included when BUILD_SHARED_LIBS is OFF.
      PRIVATE_DEPENDENCIES_CONFIGURED
      # PRIVATE dependencies that are only included when BUILD_SHARED_LIBS is
      # OFF
  ...
  if(NOT "${_PackageProject_PUBLIC_INCLUDES}" STREQUAL "")
    foreach(_INC ${_PackageProject_PUBLIC_INCLUDES})
      ...
      if(IS_DIRECTORY ${_INC})
        # the include directories are directly installed to the install
        # destination. If you want an `include` folder in the install
        # destination, name your include directory as `include` (or install it
        # manually using `install()` command).
