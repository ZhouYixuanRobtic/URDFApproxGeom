...
function(FIND_OR_BUILD_PACKAGE _pkg)
  ...
  cmake_parse_arguments(_${_PKG} "${_options}" "${_oneValueArgs}"
                        "${_multiValueArgs}" "${ARGN}")

  # Arguments for the find_package command REQUIRED and QUIET will be set when
  # necessary
  if(DEFINED _${_PKG}_COMPONENTS)
    set(_findArgs COMPONENTS ${_${_PKG}_COMPONENTS}
                  ${_${_PKG}_UNPARSED_ARGUMENTS})
  else()
    set(_findArgs ${_${_PKG}_UNPARSED_ARGUMENTS})
  endif()

  # Preliminary find_package to enable/disable USE_SYSTEM_${_PKG} option Use the
  # FindPkg.cmake module first
  ...
  option(YCM_DISABLE_SYSTEM_PACKAGES
         "Disable use of all the system installed packages" OFF)
  mark_as_advanced(YCM_DISABLE_SYSTEM_PACKAGES)
  cmake_dependent_option(
    USE_SYSTEM_${_PKG} "Use system installed ${_pkg}" ON
    "HAVE_SYSTEM_${_PKG};NOT YCM_DISABLE_SYSTEM_PACKAGES" OFF)
  ...
  elseif(CMAKE_DISABLE_BUILD_PACKAGE_${_PKG})
    ...
    # The system version was not found or the user chose not to use it, building
    # the package was not disabled, therefore we need to build it.
    ...
    if(TARGET ${_pkg})
      # Weird. We found the recipe, but this doesn't add a TARGET ${_pkg} Should
      # not happen, let's print an AUTHOR_WARNING
      message(
        AUTHOR_WARNING
          "A ${_pkg} target already exists before including Build${_pkg}.")
    endif()
    ...
    if(NOT _${_PKG}_RECIPE)
      ...
      if(NOT TARGET ${_pkg})
        ...
        message(
          AUTHOR_WARNING
            "The file ${_${_PKG}_RECIPE} does not define a ${_pkg} target.")
      endif()
      set(HAVE_${_PKG} 1)
    endif()
