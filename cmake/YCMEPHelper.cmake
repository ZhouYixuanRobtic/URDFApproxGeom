...
# _YCM_INCLUDE
#
# Internal macro to include files from cmake-next. If YCM was not found, we are
# bootstrapping, therefore we need to download and use these modules instead of
# the ones found by cmake This must be a macro and not a function in order not
# to enclose in a new scope the variables added by the included files.
...
macro(_YCM_INCLUDE _module)
  if(YCM_FOUND)
    ...
    include_url(
      ${YCM_BOOTSTRAP_BASE_ADDRESS}/cmake-next/proposed/${_module}.cmake
      ${_expected_hash_args} DOWNLOAD_ONCE STATUS _download_status)
    if(NOT _download_status EQUAL 0)
      ...
      message(
        FATAL_ERROR
          "Download failed with ${_download_status_0}: ${_download_status_1}")
    endif()
...
# _YCM_HASH_CHECK
#
# Internal function to check if a module in user repository is updated at the
# latest version and eventually print an AUTHOR_WARNING.
#
...
")
      if(WIN32)
        ...
        # On windows, the file could have windows-style EOL This should work for
        # any git configuration for core.autocrlf
...
# _YCM_SETUP
#
# Internal function to perform generic setup. This must be a macro and not a
# function in order not to enclose in a new scope the variables added by the
# included files.
...
macro(_YCM_SETUP)
  ...
  set_property(DIRECTORY PROPERTY CMAKE_PARSE_ARGUMENTS_DEFAULT_SKIP_EMPTY
                                  FALSE)
  ...
  option(YCM_EP_EXPERT_MODE
         "Enable all targets for projects in development mode" OFF)
  option(YCM_EP_MAINTAINER_MODE
         "Enable all targets for projects in development mode" OFF)
  mark_as_advanced(YCM_EP_EXPERT_MODE YCM_EP_MAINTAINER_MODE)
  ...
  if(NOT YCM_FOUND) # Useless if we don't need to bootstrap
    set(YCM_BOOTSTRAP_BASE_ADDRESS
        "https://raw.github.com/robotology/ycm/HEAD/"
        CACHE STRING "Base address of YCM repository")
    mark_as_advanced(YCM_BOOTSTRAP_BASE_ADDRESS)
  endif()

  # Install directory for all sub-projects TODO Make this a cached variable for
  # installation outside build directory
  set(YCM_EP_INSTALL_DIR
      "${CMAKE_BINARY_DIR}/install"
      CACHE
        PATH
        "Path to the superbuild installation directory. WARNING: If this path is not writable by the user, you will have to build as superuser"
  )
  mark_as_advanced(YCM_EP_INSTALL_DIR)

  set(YCM_EP_ADDITIONAL_CMAKE_ARGS
      ""
      CACHE
        STRING
        "Additional CMake arguments that are passed to all CMake subprojects of the superbuild."
  )
  mark_as_advanced(YCM_EP_ADDITIONAL_CMAKE_ARGS)

  # ExternalProject does not handle correctly arguments containing ";" passed
  # using CMAKE_ARGS, and instead splits them into several arguments. This is a
  # workaround that replaces ";" with "|" and sets LIST_SEPARATOR "|" in order
  ...
  # YCM_EP_ADDITIONAL_CMAKE_ARGS contains the specified CMake options in a
  # human-readable form, so different options are separated by spaces, while
  # lists are separated by ";", for example
  # -DYCM_EP_ADDITIONAL_CMAKE_ARGS:STRING="-DBUILD_TESTING:BOOL=ON
  # -DTYPE_SUPPORTED=ZIP;QTIFW" For this reason, first we substitute ";"  with
  # "|", and only later " " with ";"
  set(_YCM_EP_ADDITIONAL_CMAKE_ARGS ${YCM_EP_ADDITIONAL_CMAKE_ARGS})
  string(REPLACE ";" "|" _YCM_EP_ADDITIONAL_CMAKE_ARGS
                 "${_YCM_EP_ADDITIONAL_CMAKE_ARGS}")
  string(REPLACE " " ";" _YCM_EP_ADDITIONAL_CMAKE_ARGS
                 "${_YCM_EP_ADDITIONAL_CMAKE_ARGS}")
  ...
  )# Path used by cmake for finding stuff
  list(APPEND _YCM_EP_CMAKE_ARGS ${_YCM_EP_ADDITIONAL_CMAKE_ARGS})

  # Default CMAKE_CACHE_ARGS (Initial cache, forced)
  set(_YCM_EP_CMAKE_CACHE_ARGS
      "-DCMAKE_INSTALL_PREFIX:PATH=${YCM_EP_INSTALL_DIR}") # Where to do the
                                                           # installation

  if(DEFINED CMAKE_TOOLCHAIN_FILE)
    list(APPEND _YCM_EP_CMAKE_CACHE_ARGS
  ...
  if(NOT CMAKE_BUILD_TYPE STREQUAL "") # CMAKE_BUILD_TYPE is always defined
    list(APPEND _YCM_EP_CMAKE_CACHE_DEFAULT_ARGS
         "-DCMAKE_BUILD_TYPE:STRING=${CMAKE_BUILD_TYPE}") # If there is a
                                                          # CMAKE_BUILD_TYPE it
                                                          # is important to
                                                          # ensure it is passed
                                                          # down.
  ...
  if(DEFINED CMAKE_SKIP_INSTALL_RPATH)
    list(APPEND _YCM_EP_CMAKE_CACHE_DEFAULT_ARGS
         "-DCMAKE_SKIP_INSTALL_RPATH:PATH=${CMAKE_SKIP_INSTALL_RPATH}")
  endif()
  if(DEFINED BUILD_SHARED_LIBS)
    list(APPEND _YCM_EP_CMAKE_CACHE_DEFAULT_ARGS
         "-DBUILD_SHARED_LIBS:PATH=${BUILD_SHARED_LIBS}")
  endif()
endmacro()
...
function(_YCM_SETUP_GIT)
  ...
  if(NOT NON_INTERACTIVE_BUILD AND _error_code)
    message(
      FATAL_ERROR
        "Failed to get git name. Please set it with \"git config --global user.name Firstname Lastname\" or from your favorite git gui."
    )
  endif()
  ...
  if(NOT NON_INTERACTIVE_BUILD AND _error_code)
    message(
      FATAL_ERROR
        "Failed to get git email. Please set it with \"git config --global user.email name@example.com\" or from your favorite git gui."
    )
  endif()

  set(YCM_GIT_COMMIT_NAME
      "${_output_name}"
      CACHE STRING "Name to use for git commits")
  set(YCM_GIT_COMMIT_EMAIL
      "${_output_email}"
      CACHE STRING "Email address to use for git commits")
...
function(_YCM_EP_ADD_UPDATE_STEP)
  ...
  # The update step is automatically created, we just need to explicitly make it
  # a target
...
function(_YCM_EP_ADD_CONFIGURE_STEP)
  # The configure step is automatically created, we just need to explicitly make
  # it a target
...
function(_YCM_EP_ADD_TEST_STEP)
  if(DEFINED _YH_${_name}_TEST_COMMAND
    ...
    # The test step is automatically created if one of the test variables is
    # set, we just need to explicitly make it a target
    ExternalProject_Add_StepTargets(${_name} test)

    # The test target does not exist, therefore add_dependencies cannot be used.
    # Instead we add a test.
    add_test(
      NAME ${_name}_test
      COMMAND ${CMAKE_COMMAND} --build ${CMAKE_CURRENT_BINARY_DIR} --config
              ${CMAKE_CFG_INTDIR} --target ${_name}-test
      WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR})
    set_property(
      TEST ${_name}_test
      APPEND
      PROPERTY LABELS ${_name})
...
function(_YCM_EP_ADD_STATUS_STEP _name)
  ...
  if(DEFINED _cmd)
    ...
    ExternalProject_Add_Step(
      ${_name} status
      COMMAND ${CMAKE_COMMAND} -E cmake_echo_color --switch=$(COLOR) --cyan
              "Working directory: ${source_dir}" ${_cmd}
...
function(_YCM_EP_ADD_CLEAN_STEP _name)
  unset(_cmd)
  # FIXME is _YH_${_name}_CLEAN_COMMAND to the function? FIXME check if this
  # works on MSVC and Xcode

  if(NOT DEFINED _YH_${_name}_CLEAN_COMMAND OR _YH_${_name}_CLEAN_COMMAND
                                               STREQUAL "_")
    set(_cmd
        ${CMAKE_COMMAND}
        --build
...
function(_YCM_EP_ADD_DEPENDEES_STEPS _name)
  ...
  if(_depends_set)
    ...
    foreach(_dep ${_depends})
      if(TARGET ${_dep}-update) # only if the target update exists for the
                                # dependency
        ExternalProject_Add_StepDependencies(${_name} dependees-update
                                             ${_dep}-update)
      endif()
    endforeach()
  endif()
      ...
      if(TARGET ${_dep})
        ...
        if(is_ep)
          if(YCM_EP_DEVEL_MODE_${_dep} OR YCM_EP_MAINTAINER_MODE)
            ...
            # dependers-update step (update all packages that require this
            # package)
            if(TARGET ${_name}-update)
              if(NOT TARGET ${_dep}-dependers-update)
                ExternalProject_Add_Step(
                  ${_dep} dependers-update
                  WORKING_DIRECTORY ${${_dep}_BINARY_DIR}
                  COMMENT "Dependers for '${_dep}' updated."
                  EXCLUDE_FROM_MAIN 1
                  ALWAYS 1)
                ExternalProject_Add_StepTargets(${_dep} NO_DEPENDS
                                                dependers-update)
              endif()
              ExternalProject_Add_StepDependencies(${_dep} dependers-update
                                                   ${_name}-update)
            endif()
          endif()
        endif()
...
function(_YCM_EP_ADD_EDIT_CACHE_STEP _name)
  ...
  if(_${_name}_configure_command_id STREQUAL "cmake")
    ...
    ExternalProject_Add_Step(
      ${_name} edit_cache
      COMMAND ${CMAKE_EDIT_COMMAND} -H${_source_dir}${_source_subdir}
              -B${_binary_dir}
      WORKING_DIRECTORY ${_binary_dir}
      DEPENDEES configure
      EXCLUDE_FROM_MAIN 1
...
#
# Add "print-directories" step for any repository. This step prints source and
# build directories for an external project.
...
function(_YCM_EP_ADD_PRINT_DIRECTORIES_STEP _name)
  ...
  ExternalProject_Add_Step(
    ${_name} print-directories
    COMMAND ${CMAKE_COMMAND} -E echo "" ${_source_cmd}
    COMMAND ${CMAKE_COMMAND} -E echo "" ${_binary_cmd}
...
#
# Add "open" step for any repository. This step opens the external project for
# editing
...
#
# Add the project to the "install" target in the main build. This by default
# works only for CMake projects, but project using other build systems can be
# enabled by just by creating a `cmake_install.cmake` file in the build
# directory.
...
function(_YCM_EP_ADD_INSTALLATION _name)
  ...
  install(
    CODE "if(NOT CMAKE_INSTALL_LOCAL_ONLY AND EXISTS \"${_binary_dir}/cmake_install.cmake\")
...
function(YCM_EP_HELPER _name)
  ...
  if(TARGET ${_name})
    message(
      WARNING
        "Failed to add target ${_name}. A target with the same name already exists."
    )
    return()
  endif()
  # Check arguments
  ...
  cmake_parse_arguments(_YH_${_name} "${_options}" "${_oneValueArgs}"
                        "${_multiValueArgs}" "${ARGN}")
  # HACK: set(var "" PARENT_SCOPE) before CMake 3.0.0 did not set an empty
  # string, but instead unset the variable. For compatibility with older
  # versions, cmake_parse_arguments, keeps the same behaviour, therefore they
  # are not set.
  foreach(
    _step
    DOWNLOAD
    ...
    TEST
    CLEAN)
    if("${ARGN}" MATCHES ";?${_step}_COMMAND;"
       AND NOT DEFINED _YH_${_name}_${_step}_COMMAND)
  ...
  elseif(_YH_${_name}_COMPONENT STREQUAL "build")
    message(
      AUTHOR_WARNING
        "Using \"build\" is dangerous, you should choose another name for the component."
    )
  endif()
  ...
  elseif(_YH_${_name}_FOLDER STREQUAL "build")
    message(
      AUTHOR_WARNING
        "Using \"build\" is dangerous, you should choose another name for the folder."
    )
  endif()
  ...
  set(${_name}_TMP_DIR
      ${CMAKE_BINARY_DIR}/${_YH_${_name}_FOLDER}/${_name}${CMAKE_FILES_DIRECTORY}/YCMTmp
  )
  set(${_name}_STAMP_DIR
      ${CMAKE_BINARY_DIR}/${_YH_${_name}_FOLDER}/${_name}${CMAKE_FILES_DIRECTORY}/YCMStamp
  )
  ...
  set(${_name}_CMAKE_CACHE_DEFAULT_ARGS CMAKE_CACHE_DEFAULT_ARGS
                                        ${_YCM_EP_CMAKE_CACHE_DEFAULT_ARGS})
  if(_YH_${_name}_CMAKE_CACHE_DEFAULT_ARGS)
    list(APPEND ${_name}_CMAKE_CACHE_DEFAULT_ARGS
         ${_YH_${_name}_CMAKE_CACHE_DEFAULT_ARGS})
  endif()
  ...
  list(APPEND ${_name}_ALL_CMAKE_ARGS ${${_name}_CMAKE_ARGS}
       ${${_name}_CMAKE_CACHE_ARGS} ${${_name}_CMAKE_CACHE_DEFAULT_ARGS})
  ...
  if("${_YH_${_name}_COMPONENT}" STREQUAL "documentation")
    # Documentation component does not have a build step, unless specified by
    # the user. The string "_" can be used to specify that the default command
    # should be left.
  ...
  # For non-cmake project, pass PKG_CONFIG_PATH to the CONFIGURE_COMMAND in
  # order to allow pkg-config to locate the packages built and installed by YCM.
  if(DEFINED _YH_${_name}_CONFIGURE_COMMAND
    ...
    if(DEFINED ENV{PKG_CONFIG_PATH} AND NOT "$ENV{PKG_CONFIG_PATH}" MATCHES
                                        "${_regex}")
      if(WIN32)
      ...
      set(_YH_${_name}_CONFIGURE_COMMAND
          "${CMAKE_COMMAND}" -E env "PKG_CONFIG_PATH=${_pkg_config_path}"
          ${_YH_${_name}_CONFIGURE_COMMAND})
    endif()
  ...
  foreach(
    ...
    if(DEFINED _YH_${_name}_${_step}_COMMAND)
      list(APPEND ${_name}_COMMAND_ARGS ${_step}_COMMAND
           "${_YH_${_name}_${_step}_COMMAND}")
    endif()
  endforeach()
  ...
  if(DEFINED _YH_${_name}_EXCLUDE_FROM_ALL)
    list(APPEND ${_name}_EXTRA_ARGS EXCLUDE_FROM_ALL
         ${_YH_${_name}_EXCLUDE_FROM_ALL})
  endif()
  # BEGIN DEPRECATED Since YCM 0.10
  if(DEFINED _YH_${_name}_CONFIGURE_SOURCE_DIR)
    message(
      DEPRECATION
        "CONFIGURE_SOURCE_DIR is deprecated. Use SOURCE_SUBDIR instead")
    if(DEFINED _YH_${_name}_SOURCE_SUBDIR)
      message(
        FATAL_ERROR
          "CONFIGURE_SOURCE_DIR and SOURCE_SUBDIR cannot be used together")
    endif()
    set(_YH_${_name}_SOURCE_SUBDIR "${_YH_${_name}_CONFIGURE_SOURCE_DIR}")
  endif()
  # END DEPRECATED Since YCM 0.10
  if(DEFINED _YH_${_name}_SOURCE_SUBDIR)
    list(APPEND ${_name}_EXTRA_ARGS SOURCE_SUBDIR
         "${_YH_${_name}_SOURCE_SUBDIR}")
  endif()
  ...
  if("${_YH_${_name}_TYPE}" STREQUAL "GIT")
    ...
    list(
      APPEND ${_name}_REPOSITORY_ARGS GIT_REPOSITORY
      ${YCM_GIT_${_YH_${_name}_STYLE}_BASE_ADDRESS}${_YH_${_name}_REPOSITORY})
    ...
    if(YCM_GIT_${_YH_${_name}_STYLE}_COMMIT_NAME)
      ...
      set(_setup_repo_cmd
          ...
          --local
          user.name
          ${YCM_GIT_${_YH_${_name}_STYLE}_COMMIT_NAME})
    endif()
    ...
    if(YCM_GIT_${_YH_${_name}_STYLE}_COMMIT_EMAIL)
      set(_setup_repo_cmd
          ...
          --local
          user.email
          ${YCM_GIT_${_YH_${_name}_STYLE}_COMMIT_EMAIL})
    endif()
  elseif("${_YH_${_name}_TYPE}" STREQUAL "SVN")
    ...
    list(
      APPEND ${_name}_REPOSITORY_ARGS SVN_REPOSITORY
      ${YCM_SVN_${_YH_${_name}_STYLE}_BASE_ADDRESS}${_YH_${_name}_REPOSITORY})

    if(YCM_SVN_${_YH_${_name}_STYLE}_USERNAME)
      list(APPEND ${_name}_REPOSITORY_ARGS SVN_USERNAME
           ${YCM_SVN_${_YH_${_name}_STYLE}_USERNAME})
    endif()

    if(YCM_SVN_${_YH_${_name}_STYLE}_PASSWORD)
      list(APPEND ${_name}_REPOSITORY_ARGS SVN_PASSWORD
           ${YCM_SVN_${_YH_${_name}_STYLE}_PASSWORD})
    endif()

    if(DEFINED _YH_${_name}_TRUST_CERT)
      list(APPEND ${_name}_REPOSITORY_ARGS SVN_TRUST_CERT
           ${_YH_${_name}_TRUST_CERT})
    endif()
  elseif("${_YH_${_name}_TYPE}" STREQUAL "HG")
    # Specific setup for Mercurial
    _ycm_setup_hg()

    list(APPEND ${_name}_REPOSITORY_ARGS HG_REPOSITORY
         ${YCM_HG_${_YH_${_name}_STYLE}_BASE_ADDRESS}${_YH_${_name}_REPOSITORY})
  ...
  option(YCM_EP_DEVEL_MODE_${_name}
         "Enable development targets for the \"${_name}\" project" OFF)
  ...
  if(NON_INTERACTIVE_BUILD AND YCM_EP_DEVEL_MODE_${_name})
    message(
      SEND_ERROR
        "NON_INTERACTIVE_BUILD AND YCM_EP_DEVEL_MODE_${_name} cannot be used at the same time"
    )
  endif()
  ...
  if(NOT "${_YH_${_name}_COMPONENT}" MATCHES
  ...
  set_property(GLOBAL PROPERTY _YCM_${_name}_COMPONENT
...
#
# This function writes a Project.xml file that can be read and interpreted by
# CDash.
...
function(YCM_WRITE_CDASH_PROJECT_FILE _filename)
  ...
  if(NOT DEFINED CTEST_PROJECT_NAME OR CTEST_PROJECT_NAME STREQUAL "")
    message(
      SEND_ERROR
        "Cannot generate Project.xml. Please set CTEST_PROJECT_NAME variable or add a ${CMAKE_SOURCE_DIR}/CTestConfig.cmake file"
    )
  endif()
  ...
  foreach(_proj_label ${_projects})
    ...
    foreach(_dep_label ${_dependencies})
      ...
      if(_is_ycm EQUAL -1)
        ...
        get_property(
          _dep_component_set GLOBAL
          PROPERTY _YCM_${_dep_label}_COMPONENT
          SET)
        get_property(_dep_component GLOBAL
                     PROPERTY _YCM_${_dep_label}_COMPONENT)
...
")
  ...
  list(REMOVE_ITEM _components "external" "documentation" "templates"
       "examples")
...
# Bootstrap YCM.
#
# If the variable YCM_BOOTSTRAP_VERBOSE is set it prints all the output from the
# commands executed.
...
macro(YCM_BOOTSTRAP)
  ...
  execute_process(
    COMMAND
      ${CMAKE_COMMAND} -P
      ${YCM_BINARY_DIR}/${CMAKE_FILES_DIRECTORY}/YCMTmp/YCM-gitclone.cmake
      ${_quiet_args}
  ...
  execute_process(
    COMMAND
      ${CMAKE_COMMAND} -P
      ${YCM_BINARY_DIR}/${CMAKE_FILES_DIRECTORY}/YCMTmp/YCM-gitupdate.cmake
      ${_quiet_args}
    RESULT_VARIABLE _result)
  if(_result)
  ...
  file(READ ${YCM_BINARY_DIR}/${CMAKE_FILES_DIRECTORY}/YCMTmp/YCM-cfgcmd.txt
       _cmd)
  string(STRIP "${_cmd}" _cmd)
  string(REGEX REPLACE "^cmd='(.+)'" "\\1" _cmd "${_cmd}")
  # The -DCMAKE_PREFIX_PATH in YCM-cfgcmd.txt uses | as list separator, so it is
  # not usable as it is when invoking CMake from the config line. As YCM during
  # bootstrap does not need to find any package via CMAKE_PREFIX_PATH, we just
  # remove it
  ...
  string(
    REGEX
    REPLACE "-C.+\\.cmake;"
            "${_YCM_EP_CMAKE_CACHE_ARGS};${_YCM_EP_CMAKE_CACHE_DEFAULT_ARGS};"
            _cmd "${_cmd}")
  # The command line contains location tags, therefore we need to expand it.
  _ep_replace_location_tags(YCM _cmd)
  execute_process(
    COMMAND ${_cmd}
    WORKING_DIRECTORY ${YCM_BINARY_DIR} ${_quiet_args}
    RESULT_VARIABLE _result)
  if(_result)
    message(FATAL_ERROR "Cannot configure YCM repository")
  endif()

  # On multi-config generators (MSVC and Xcode) always build in "Release"
  # configuration
  ...
  execute_process(
    ...
    WORKING_DIRECTORY ${YCM_BINARY_DIR} ${_quiet_args}
    RESULT_VARIABLE _result)
  # If uninstall fails, YCM was not previously installed, therefore do not fail
  # with error
  ...
  if(CMAKE_DISABLE_FIND_PACKAGE_YCM)
    # We need to disable this flag, in case the the user explicitly enabled it
    # in order to use the bootstrapped version, otherwise the next find_package
    # will fail.
    set(CMAKE_DISABLE_FIND_PACKAGE_YCM FALSE)
  endif()
  find_package(YCM PATHS ${YCM_INSTALL_DIR} NO_DEFAULT_PATH)

  # Reset YCM_DIR variable so that next find_package will fail to locate the
  # package and this will be kept updated
  set(YCM_DIR
      "YCM_DIR-NOTFOUND"
      CACHE PATH "The directory containing a CMake configuration file for YCM."
            FORCE)
